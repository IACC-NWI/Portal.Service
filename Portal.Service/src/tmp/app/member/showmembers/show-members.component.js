"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var show_members_service_1 = require('./show-members.service');
var ShowMembersComponent = (function () {
    function ShowMembersComponent(showMemberService) {
        this.showMemberService = showMemberService;
        this.members = new Array();
    }
    ShowMembersComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.showMemberService.getAllMembers().subscribe(function (members) { return _this.members = members; });
    };
    ShowMembersComponent.prototype.lookupMember = function () {
        var _this = this;
        if (this.memberPhoneNumber !== '') {
            this.showMemberService.lookupMemberUsingPhoneNumber(this.memberPhoneNumber)
                .subscribe(function (members) { return _this.members = members; });
        }
    };
    ShowMembersComponent = __decorate([
        core_1.Component({
            selector: 'iacc-show-members',
            templateUrl: 'app/member/showmembers/show-members.html'
        }), 
        __metadata('design:paramtypes', [show_members_service_1.ShowMembersService])
    ], ShowMembersComponent);
    return ShowMembersComponent;
}());
exports.ShowMembersComponent = ShowMembersComponent;

//# sourceMappingURL=show-members.component.js.map
